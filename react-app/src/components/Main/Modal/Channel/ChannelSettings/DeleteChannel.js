...
const DeleteChannel = ({ channel, onClose }) => {
	...
	const handleSubmit = async (e) => {
		e.preventDefault();
		const channelToDelete = { id: channel.id, name };
	...
	return (
		<form className="setting-server-overview-wrap" onSubmit={handleSubmit}>
			...
			<div className="setting-delete-server-waring">
				...
				<span className="bold">{channel?.name}</span>? This action cannot be
				undone.
			</div>
			<label>
				ENTER CHANNEL NAME
			...
			<div className="setting-button-wrap">
				...
				<button className="form-create-btm-btn-red" type="submit" onClick={handleSubmit}>
					Delete Channel
				</button>
			</div>
		</form>
